...
- Exposes the app's own engines as read-only tools, each answering from what the Node app
  ...
  `cx_edge_outcomes`, with the gate's stored persistence and slower-leg Div/h),
  `get_craft_margins` (`craft_margin_reports`, gated like the Craft tab), `get_snipe_report`
  (`autosnipe_report`, only for the league the scanner runs in) and `get_farm_advice`
  ...
  reads `CRAFT_MARGIN_INTERVAL_MIN` (default 10, same as the web app); set it in the Coach
  environment too if the web app overrides it.
