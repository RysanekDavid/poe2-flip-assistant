...
Patch stamp: current as of **0.5.3–0.5.4**, Runes of Aldur league (launched 2026-05-29). Desecration/Abyss is the 0.5 flagship crafting system, carried over and iterated from its 0.3 introduction (originally "Rise of the Abyssal" league) through 0.4 ("Fate of the Vaal") into 0.5. Where a fact is only confirmed for an older patch and no 0.5 source contradicts it, it's noted as legacy-but-presumed-current.

Sources are cited inline. Note: `poe2wiki.net` and `mobalytics.gg` blocked direct fetch (403) during research — facts attributed to them come from search-engine snippet synthesis, not a direct page read, and are downgraded to [single-source] accordingly even when the underlying wiki likely has broad community confirmation.
...
General flavor (broadly [CONFIRMED] across sources, though sources differ slightly on the "element" framing vs the "role" framing — reconciled below):

| Lich | Forcing Omen | Thematic bias | Notes |
|---|---|---|---|
| **Ulaman, Sovereign of the Well** | Omen of the Sovereign | Offensive: crit, elemental/physical damage, weapon & jewellery mods | Physical-damage-heavy boss fight; controls the "offensive" pool [CONFIRMED — gamerant.com, timesaver.gg ulaman-sovereign guide] |
| **Amanamu, Liege of the Lightless** | Omen of the Liege | Defensive: life, ES/armour, shields/body armour-leaning | Void-cloud mechanic on his rares (see Bosses below) [CONFIRMED — conquestcapped.com, timesaver.gg amanamu-farming-guide] |
| **Kurgal, the Blackblooded** | Omen of the Blackblooded | Ailments: poison, bleed, ignite, DoT scaling — "the niche one" | Delve/Azurite Mine depth-90+ boss variant also exists (legacy PoE1-style Kurgal encounter reused) [CONFIRMED — gamerant.com, search synthesis of poe2wiki Kurgal page] |

Two sources disagreed on whether Ulaman/Amanamu/Kurgal map cleanly to fixed elements (Lightning/Cold/Fire) or to roles (offense/defense/ailment) — later, more detailed sources (gamerant.com's per-slot tables, timesaver's boss guides) consistently support the **role-based** framing (offense/defense/ailment), which should be treated as the more reliable read; the strict elemental mapping appears to be an oversimplification from an earlier/thinner source. [reconciled from conflicting single-sources]
...
4. **Craft-and-flip weapon/jewellery bases with forced Lich mods** using the Sovereign/Liege/Blackblooded omens — since these mods are otherwise unattainable, a well-rolled Ulaman/Amanamu/Kurgal mod on a good base (e.g., the bow Attack Speed / Pierce / Chain-affecting rolls found above) is a chase item for the archetypes that want it (crit/ele attackers → Ulaman; ES/armour stackers → Amanamu; poison/bleed/ignite → Kurgal).
...
- **REFUTED** — 9. Ulaman/Amanamu/Kurgal role mapping (offense/defense/ailment) reconciled over a conflicting elemental mapping (Lightning/Fire/Cold).
  → The reconciliation leans the wrong way. The elemental mapping is the one directly substantiated by primary data: poe2wiki's full modifier list shows Amanamu prefixes penetrating/scaling Fire (Ignite Magnitude, 'Penetrate Fire Resistance'), Kurgal prefixes penetrating/scaling Cold (Freeze Buildup, 'Penetrate Cold Resistance'), and Ulaman prefixes penetrating/scaling Lightning (Shock Magnitude, 'Penetrate Lightning Resistance') — i.e., Amanamu=Fire, Kurgal=Cold, Ulaman=Lightning/Physical, matching the 'conflicting' single source, not contradicting it. The offense/defense/ailment framing (used by some guide sites to describe the Liege/Sovereign/Blackblooded omens) is a loose thematic gloss that doesn't hold up against the granular mod list: Kurgal's own mods are mostly Cold/spell/energy-shield, not ailment-themed, while ailment mods (Bleed Magnitude, 'Magnitude of Damaging Ailments') actually sit under Ulaman. The KB should drop the offense/defense/ailment framing as inaccurate/oversimplified and keep the elemental mapping as primary. (https://www.poe2wiki.net/wiki/List_of_desecrated_modifiers)
