...
import { insertSnapshots, latestSnapshots, itemSpark } from "../db/queries";
...
ok("first insert writes 2 rows", insertSnapshots([item("a", 10, 100), item("b", 20, 200)]) === 2);

// 2. same PRICE, different volume + change → deduped (the old bug inserted these)
ok("same price, diff volume → 0 new rows", insertSnapshots([item("a", 10, 999, [9], 99), item("b", 20, 888, [8], 88)]) === 0);

// 3. price change → 1 new row (and fresh spark [7,7]/77 upserted for a)
ok("price change → 1 new row", insertSnapshots([item("a", 11, 100, [7, 7], 77), item("b", 20, 200)]) === 1);
